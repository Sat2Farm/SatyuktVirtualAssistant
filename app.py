...
if "vector_store" in st.session_state:  # Only show chat if vector store is initialized
    ...
    st.info(
        "🔄 Initializing Satyukt Virtual Assistant... Please wait a moment.")  # Message if vector_store is not yet initialized
...
# below is for testing and using voice features in it
# import streamlit as st
# import os
# import pdfplumber
# import tempfile
...
# }
#
# # Custom CSS for agriculture theme (enhanced with voice features)
# st.markdown(
#     """
#     <style>
#     /* Main app styling */
...
#     def speak_thread():
#         try:
#             if tts_engine and st.session_state.tts_enabled:
...
# def listen_for_voice_input(language="en-US"):
...
#
#
# # Sidebar for language selection and voice controls
# with st.sidebar:
#     st.markdown('<div class="language-selector">', unsafe_allow_html=True)
#     st.markdown('<div class="language-label">🌍 Select Language / भाषा चुनें</div>', unsafe_allow_html=True)
#
...
#
#     # TTS Toggle
#     tts_enabled = st.checkbox("🔊 Enable Text-to-Speech", value=st.session_state.tts_enabled)
#     st.session_state.tts_enabled = tts_enabled
#
#     # Voice Input Button
#     if st.button("🎤 Start Voice Input", key="voice_input_btn"):
#         if speech_recognizer:
#             st.session_state.is_listening = True
#             voice_lang = voice_language_mapping.get(selected_lang, "en-US")
#
#             with st.spinner("🎤 Listening... Please speak now"):
#                 voice_text = listen_for_voice_input(voice_lang)
...
#                     st.success(f"Voice input captured: {voice_text}")
#                     # Automatically set the voice input as the user prompt
#                     st.session_state.auto_voice_input = voice_text
#                 else:
...
#         st.markdown(f"**Last voice input:** {st.session_state.voice_input_text}")
#
#     # Sidebar info
#     st.markdown("---")
#     st.markdown("### 🌾 About Satyukt 🌾")
...
#     <div class="welcome-container">
#         <div class="logo-title">🌾 Satyukt Virtual Assistant 🌾</div>
#         <div class="welcome-subtitle">Empowering Agriculture with Satellite Intelligence, AI Technology & Voice Interaction</div>
#     </div>
#     """,
#     unsafe_allow_html=True
# )
...
#         <div style="background: rgba(139, 195, 74, 0.1); padding: 20px; border-radius: 10px; text-align: center; margin: 10px 0;">
#             <div style="font-size: 2em; margin-bottom: 10px;">🎤</div>
#             <div style="font-weight: 600;">Voice Interaction</div>
#         </div>
#         """,
#         unsafe_allow_html=True
#     )
...
#
# def is_out_of_context(answer, current_selected_lang):
#     """Check if the answer is out of context"""
#     contact_message_template = contact_messages.get(current_selected_lang, contact_messages['English']).lower()
#
#     # Check for direct match (case-insensitive)
#     if answer.strip().lower() == contact_message_template:
...
# i wanted to use voice features if any one uses use mic to say something convert it to the text and feed as usual is taken in above code and whatever answer given by the chatbot read it(text to speech)